...
function Tcll5850_BBCPP1_parse(type, node) {
    ...
    //--- [copy] ---:
    if (window.Tcll5850_BBCPP1_Settings[3]) {
        var post = node.html();
        post = post.replace(/\[copy\](.+?)\[\/copy\]/g, function(BBC,url) {
			...
			req.open("GET", url+'?download=1', false);
            req.send();
...
$(document).ready(function() {
    ...
    for (var i=0; i<bbc_features.length; i++) {
        switch (bbc_features[i]) {
            ...
            case "copy": window.Tcll5850_BBCPP1_Settings[3] = true;
        };};
